...
## >> Revision ####
x <- c("gyrocochlea_janetwaterhouseae", "acanthokara_kaputensis", 
       "pseudococcus_markharveyi", "zephyrarchaea_austini", "ecnomina_manicula")
y <- gsub('[[:digit:]]+', '', data_sp)
y <- gsub("_$", "", y)

## Find species in data
all(x %in% y)
which(!x %in% y)
x <- data_sp[grep(paste0(x, collapse = "|"), data_sp)]

xdat <- data[spfile %in% x]; dim(xdat)
length(unique(xdat$spfile))
length(unique(xdat$scientificName))

write.csv(xdat, file = file.path(output_dir, "fig1_spdata_all.csv"), 
          row.names = FALSE)
...
## Species with more than 3 records 
xdat[, .N, spfile][N>=3]$spfile
bugs <- xdat[, .N, spfile][N>=3]$spfile

## Write species shapefiles
shapefile_dir <- file.path(output_dir, "Fig1_shapefiles")
